          ...
          <AdComponent/>
              <div id="weapons-flex">
                  {armasPT.map((post: any, i: number) => (
                    <a href={`/en/artifacts/${data[i].replace(/'/g, '').toLowerCase().replace(/ /g, '-')}`} key={i} className={`weapon-card ${post.name.toLowerCase().replace(/\s+/g, '-')} rarity-${post.rarityList[post.rarityList.length - 1]}-weapon`}>
